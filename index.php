...
function getLoggedInClient(){
    $topUrl = 'https://hitpo.it-hiroshima.ac.jp/PfStudent/Login?target_url=%2fPfStudent%2fPortal';
    $cli = new Client();
    ...
    // $cookies = $cli->getCookieJar()->set(new Cookie('__AntiXsrfToken',));
    // print_r($cookies);
    // exit;
    $session = $top->filter('div.aspNetHidden')->each(function($node){
        $node->filter("input")->each(function($inputform){
